...
class ExpenseServiceTests {
	 ...
	 * @Test
	 * 
	 * @DisplayName("Find Expense By Income ID") void
	 * arrangeExpenseObject_actfindExpemse_assertCheckExpenseIsCorrect() {
	 * List<Expense> expenses = new ArrayList<>(); expenses.add(expense);
	 * 
	 * 
	 ...
	 * 
	 * 
	 * Integer incomeId = 1; // arrange
	 * when(MockExpenseRepository.findByIncomeId(incomeId)).thenReturn(expenses); //
	 * act List<Expense> result =
	 * MockExpenseService.findExpensesByIncomeId(incomeId);
	 * 
